...
function resize() {
	...
	jQuery('.mur:before').hide();

	jQuery('.mur:before').css('border-left-width', h); */

	/*var a = jQuery('.mur').width();
	var b = jQuery('#sol').height();
	var c = Math.pow(a,2)+Math.pow(b,2);

	c = Math.sqrt(c);

	jQuery('head').append('<style>.mur:before{width:'+c+'px !important;}</style>');

	//jQuery('.mur::before').width(c);
