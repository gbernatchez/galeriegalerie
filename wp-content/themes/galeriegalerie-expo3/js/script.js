...
jQuery(document).ready(function(){
    ...
    jQuery('.page-template-guestbook').parent().removeClass('scene');
    jQuery('.page-template-page-adriana').parent().removeClass('scene');
    jQuery('.page-template-page-megane').parent().removeClass('scene');
    ...
    jQuery('.livre').click(function(){
        event.preventDefault();
        jQuery('.bipbip').addClass('visible');
    });

	resize();
    jQuery(window).resize(function(){ resize(); });

	jQuery('.subtitle > a').click(function(){
		event.preventDefault();
	})
    ...
    jQuery('.bt-chatroom').click(function(){
        ...
        jQuery('.bt-chatroom span').text(text == "Fermer le chat roume" ? "Ouvrir le chat roume" : "Fermer le chat roume");
        event.preventDefault();
    });

    if(jQuery('body').hasClass('logged-in') && jQuery('body').hasClass('page-template-page-entree-vernissage')) {
        jQuery('#zone-chatroom').addClass('visible');
        jQuery('.bt-chatroom span').text("Fermer le chat roume");
        jQuery('#zone-chatroom input[type="text"].wcInput').focus();

    }

    jQuery('.close-login').click(function(){
        jQuery('#zone-chatroom #login, #zone-chatroom #register').hide();
        event.preventDefault();
    });

    jQuery("#getting-started").countdown("2017/01/28 16:00:00", function(event) {
    jQuery(this).text(
...
function resize() {
    if(isMobile) {
        if(window.innerHeight > window.innerWidth){ 
            ...
            //jQuery('#zone-chatroom').removeClass('turnup');
		} else { 
            jQuery('.landscape-gif').hide();
            //jQuery('#zone-chatroom').addClass('turnup');
		}
	}
}